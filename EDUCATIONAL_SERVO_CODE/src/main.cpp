...
void loop() {
  ...
  if(millis() - t > loopPeriod){
    ...
    parameter.setPoint = map(analogRead(SETPOINT_PIN),0,694,0,1023); constrain(parameter.setPoint,0,1023);
    parameter.KP = mapFloat(analogRead(KP_PIN),0,694,0,100);
    ...
    parameter.feedback = movingAverageFilter(TACHO_PIN,150);

    parameter.error = parameter.setPoint - parameter.feedback;
    parameter.totalError += parameter.error;

    if (parameter.totalError >= parameter.maxControl) parameter.totalError = parameter.maxControl;
    else if (parameter.totalError <= parameter.minControl) parameter.totalError = parameter.minControl;
    ...
    if(parameter.controlSignal >= parameter.maxControl) parameter.controlSignal = parameter.maxControl;
    else if(parameter.controlSignal <= parameter.minControl) parameter.controlSignal = parameter.minControl;
    ...
    lcd.setCursor(0,0);
    lcd.print("SP:");lcd.print(parameter.setPoint);
    lcd.setCursor(0,1);
    lcd.print("PV:");lcd.print(parameter.feedback);
    lcd.flush();
